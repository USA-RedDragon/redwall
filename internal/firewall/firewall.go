...
func (f *Firewall) writeIP(newIP net.IP) {
	file, err := os.OpenFile("/etc/nftables/public_ip.nft", os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0755)
	if err != nil {
		klog.Fatal(err)
	}
	_, err = file.WriteString(fmt.Sprintf("define public_ip = %s\n", newIP.String()))
	if err != nil {
		klog.Fatal(err)
	}
